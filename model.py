...
class CycleGAN(nn.Module):
    def __init__(self,in_channels,out_channels,nker=64,norm="inorm"): #instance normalization
        ...
        res=[]
        for i in range(9):
            res +=[ResBlock(4*nker,4*nker,kernel_size=3,padding=(3-1)//2,norm=2,relu=0.0,stride=1)]
...
class Pix2Pix(nn.Module):
    def __init__(self,in_channels, out_channels, nker=64,norm="borm"):
        super(Pix2Pix, self).__init__()

        self.enc1=CBR2d(in_channels,1*nker, kernel_size=4,padding=1,
                        norm=None,relu=0.2,stride=2)
        self.enc2 =CBR2d(1*nker,2*nker,kernel_size=1,padding=1,
        ...
        self.dec4=DECBR2d(2*8*nker,8*nker,kernel_size=4,padding=1,
                          norm=norm,relu=0.0,stride=2)
        self.dec5=DECBR2d(2*8*nker,4*nker,kernel_size=4,padding=2,
                          norm=norm,relu=0.0,stride=2)
        self.dec6=DECBR2d(2*4*nker,8*nker,kernel_size=4,padding=1,
                          norm=norm, relu=0.2,stride=2)
        self.dec7=DECBR2d(2*2*nker,2*nker,kernel_size=4,padding=1,
                          norm=norm, relu=0.2,stride=2)
    ...
    def forward(self,x):
        ...
        cat5=torch.cat((dec4,dec4),dim=1)
        dec5=self.dec5(cat5)

        cat6=torch.cat((dec5,dec3),dim=1)
        dec6=self.dec6(cat6)

        cat7=torch.cat((dec6,dec2),dim=1)
        dec7=self.dec7(cat7)

        cat8=torch.cat((dec7,dec1),dim=1)
        dec8=self.dec8(cat8)
...
class DCGAN(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm="bnorm"): #100,3
        super(DCGAN, self).__init__()

        self.dec1 = DECBR2d(1 * in_channels, 8 * nker, kernel_size=4, stride=1,
                            padding=0, norm=norm, relu=0.0, bias=False) #Leaky relu
...
class Discriminator(nn.Module): #따로 바꾸지않아도됨
    def __init__(self, in_channels, out_channels, nker=64, norm="bnorm"): #3,1
